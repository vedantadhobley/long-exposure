...
> **PRE-LAUNCH TRANCHE (the cheap subset — wired 2026-05-27).** A thin slice of Tier-1 is
> wired now: stats that are pure arithmetic over data the scorer already holds, or one bulk
> baseline read — **no new computation infrastructure**. This is NOT the full per-type §3.3
> vision; that's the wave below.
> - ✅ `Analytics` package (`slippageBps`, `oneSidedness`, `pctOfBaseline`, `robustZ`, `median`, `mad`, `percentileRank`) + 7 unit tests — committed.
> - ✅ `CaggBaselineProvider.trailingVolumeWindows()` (raw per-symbol window arrays for z/MAD/percentile) — committed.
> - ✅ `large_trade` → `pct_of_baseline_volume`; `sweep` → `slippage_bps`+`slippage_direction`; `volume_deviation` → `robust_z`+`percentile_rank`. Compiles clean; **needs the one-day test (#90) before the overnight relaunch.**
> - ⚠️ **`one_sidedness` dropped from the cluster scorers as degenerate** — `layering`/`post_cancel_cluster` cluster by `(symbol, side)` so it's ≡ 1.0 (a clustering-key artifact). The proposed swap to `median_cancel_lifetime_ms` was *also* moot — both scorers already emit median lifetime. So only 3 of 8 types get a *new* differentiating stat in this tranche. See analytics-catalog §2.8 + §3.3 "Wiring status".
> - **Cheap-now extensions (decision pending, ~1 hr + tests):** **burstiness (Fano)** → post_cancel + layering, **inter-fill cadence CV** → iceberg. Both pure math over timestamps already in the cluster → would bring the count to 6 of 8 types. Only `halt` + `liquidity_withdrawal` then wait on the infra-heavy wave.
> - 🔭 **The genuinely-valuable one-sidedness home:** `liquidity_withdrawal` side-split ("makers pulled 95% of the bid 200 ms before the halt") — needs an `order_lifecycle` side-join (delete rows lack side); changes what the scorer reads, so **test before any 10-hr run** → post-launch fast-follow, NOT launch-night.
