...
> **PRE-LAUNCH ANALYTICS — STATUS (2026-05-27).** Decisions + reasoning in `decisions.md`
> 2026-05-27 "Analytics wave"; narration-set in `analytics-catalog.md` §3.6. Sequencing: doing the
> **narration-set + a single-day rerun (option B)** to SEE the payoff before the book-replay build.
> - ✅ **`Analytics` pure-fn layer** (+ tests): slippage, one-sidedness, pct-of-baseline, robust-z/median/MAD/percentile, **reversion, Fano burstiness, CV, effective-spread, order-to-trade**. Committed (`1e2d876`).
> - ✅ **7 of 8 scorers carry a new breakdown stat** (committed `1e2d876` + `188fb58`): sweep `slippage_bps`, large_trade `pct_of_baseline_volume`, volume_deviation `robust_z`+`percentile_rank`, post_cancel/layering `burstiness_fano`, iceberg `refill_cadence_cv`, liquidity_withdrawal two-sidedness. Validated on 05-22 (full coverage, sane ranges).
> - ✅ **`EnrichAnalyticsActivity`** (post-select, lazy for ~90–170 events; `jsonb ||` idempotent) — home for DB-dependent breakdown stats. First stat: liquidity_withdrawal two-sidedness via `order_lifecycle.side` (detection unchanged on `orders_delete`). Wired into `ScoreWorkflow` after select.
> - ✅ **`one_sidedness` dropped on cluster scorers as degenerate** (clustered by side ≡ 1.0); re-homed to liquidity_withdrawal as two-sidedness (above). See analytics-catalog §2.8.
> - 🔄 **Narration-set** (`BlueprintExtractor` `extract-v2-narration-set`) — per-scorer headline guidance + framing rules (robust_z≠σ; categorical side-class). **Single-day before/after rerun on 05-22 in progress** to validate the lead/weave/drill split (catalog §3.6, first-draft).
> - 📋 **Book-replay tier (deferred per option B; design locked).** Single decode-only DPLS pcap pass through the validated `OrderBookManager`, snapshot at event timestamps → sweep effective-spread, layering depth-from-touch, halt pre-event spread, liquidity_withdrawal %-of-book+recovery. Pcaps present for all 11 relaunch days. Lands in `EnrichAnalyticsActivity`. **Only `halt` gains its first stat here.**
> - 📋 **INTERPRET-tier window stats (not built):** reversion, OFI, surrounding-VWAP → computed in `InterpretEventActivity` (reuses its ±60 s windows), prompt-resident. Come with the INTERPRET prompt phase.
> - ⚠️ **`implied_reserve` (iceberg) needs a defensible definition** before wiring — it's an *estimate* of hidden size; a confident fabricated number cuts against grounding. Propose a conservative lower-bound + label it.
> - 🎯 **Full 11-day relaunch = ONE batch** once the field set is final (book-replay + INTERPRET stats folded in), incremental via the `event_hash` skip.
