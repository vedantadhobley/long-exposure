...
- [ ] **Number misattribution in SYNTHESIZE (verifier-uncatchable).** Observed 2026-05-28 in new v6 05-13 synthesis: "TQQQ which recorded ten distinct order-deletion events" — TQQQ actually had 20 liquidity_withdrawals. The verifier passed because "10" IS in the haystack (as the day's halt count) — just not for TQQQ's liquidity withdrawals. **Class of bug**: the verifier checks number *presence* in haystack, not number-claim *correspondence*. Model is grabbing a real number from a different context and misattributing. Structurally hard to fix without LLM-as-judge:
   - Option A: Restructure the SYNTHESIZE haystack so per-symbol per-scorer counts are tagged ("TQQQ.liquidity_withdrawal: 20") — verifier checks if a number appears tagged with the claimed (symbol, scorer) pair. Possible but invasive.
   - Option B: Tighten the SYNTHESIZE prompt to cite per-symbol counts using the by_scorer / by_symbol JSON aggregates directly: "When citing a count for a specific symbol, copy the value from the day_aggregates JSON; do not derive from interpretation prose."
   - Option C: Accept misattribution as known limitation; document that synthesis paragraphs may misattribute counts (~5-10% rate). Saturday's overnight will produce different misattributions due to sampling variance — not always the same wrong number.
   *Recommend B before Saturday* — one prompt change, no code, model should follow the instruction. Test on 1 day post-launch. Punt A as post-launch.
