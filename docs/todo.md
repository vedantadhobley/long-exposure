...
Both surfaced auditing the first full-pipeline backfill day (2026-05-22: DESCRIBE 159/160, INTERPRET 156/160). They raise verifier pass rates without prompt hacking — the right move is more/cleaner data in the breakdown, not new prompt rules. **Deferred until the backfill finishes**, because either change alters breakdown bytes / pre-formatted values, which changes `event_hash` and re-narrates everything — pointless to do mid-run.

- [ ] **Add `orders_per_second` to the layering breakdown.** The single DESCRIBE failure on the day was RKLB layering: the model wrote "351.5 orders per second" (correct — 131 orders ÷ 372.7 ms) but the rate isn't a breakdown field, so the verifier flagged it as ungrounded. `LayeringScorer` already pre-computes `orders_per_level` etc. via DETECT enrichment; add `orders_per_second` (= order_count / (duration_ns / 1e9)) alongside them so the model reads it instead of computing it. (DGP layering passed with "1,189.6 orders per second" only because that value happened to be groundable; RKLB's didn't — inconsistent, and the fix removes the inconsistency.) Same class of fix that eliminated the arithmetic-error mode for the other ratios. While here, sanity-check the INTERPRET notional failures (NOW/TQQQ `545735.82`/`572846.45`, TQQQ `78.2413`, R `92`) — these are computed notionals/prices in the interpretation prose; decide whether the source breakdown should carry the notional so INTERPRET reads rather than multiplies. **Cross-day audit refinement (05-20/05-21/05-22):** iceberg is the dominant offender — 3 of 6 DESCRIBE/INTERPRET failures across the three audited days are iceberg events where the model computes a round total (OXY `12,000`, JHX `60,000`, JHX `3,721`) by multiplying `fills × median_fill_size`. **Pre-compute the iceberg total (`total_shares`) in `IcebergScorer`'s breakdown** so the model reads rather than multiplies — highest-leverage single fix, would clear the most failures. Otherwise the failure class is stable and rare (~1–2% per stage per day), all correctly caught by the verifier.
- [ ] **Number-formatting pass in `BreakdownFmt`.** Pre-format every integer ≥ 1,000 with thousand separators and every dollar value as `$X,XXX.XX` before the breakdown reaches `BlueprintExtractor` — so the LLM copies the formatted value verbatim instead of emitting raw forms. Observed on 2026-05-22: SPY liquidity_withdrawal rendered `notional value of 1047129.4 dollars` (should be `$1,047,129.40`); the halt `55.8 percentage of regular session` reads awkwardly (pre-format as `55.8%` or supply a `_label` the prose can use as "55.8 percent of the regular session"). Cosmetic but visible — inconsistent formatting across narrations reads less professional. Already noted as limitation #3 in `scoring-and-narration.md`; this is the concrete action. NOTE: changing pre-formatted breakdown values changes `event_hash` → full re-narration, so batch it with the orders_per_second change in one cache-busting pass.
