...
- [ ] **🚩 AGGREGATE streak-length fabrication (LAUNCH-RELEVANT — found 2026-05-27).** The 05-18 weekly rollup has exactly **1** prior week (05-11) yet claims *"a fifth consecutive week"* and *"the third straight week where TQQQ…"* — fabricated streak length, unsupported by the 1 prior week available. `SynthesisVerifier` passed it because the streak is a **written ordinal** ("fifth"/"third") + a qualitative claim, not a digit-number it grounds. (05-11, with 0 priors, is clean — the no-priors guard works; the bug is specifically *over-extrapolating* when ≥1 prior exists.) Visible + embarrassing at a 2-week launch ("how is it the fifth consecutive week when you launched with two?"). **Fix (two parts, both fold into tonight's analytics relaunch which re-aggregates anyway):** (1) **prompt** — pass the actual prior-week *count* into the AGGREGATE prompt and bound trend language to it ("you have N prior weeks; the strongest streak claim is N+1 weeks; with 1 prior, say 'a second consecutive week' / 'vs last week', never more"); (2) **verifier** — extend `SynthesisVerifier` (the shared AGGREGATE verifier) to detect "Nth consecutive/straight week" (digit + word ordinals) and flag when N > prior-week-count + 1. The streak risk is *highest now* (least data) and shrinks as weeks accumulate. Until fixed, the 05-18 rollup is a known-bad row.
