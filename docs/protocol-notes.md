...
- **2026-05-11 — Real-feed message-type distribution from 2026-05-08.** Full 9.5 GB TOPS file decoded end-to-end produced ~295M messages with this distribution: Q Quote Update 285M (97%), T Trade Report 7.75M, I Retail Liquidity Indicator 1.84M, P Short Sale Price Test 13.5K, H Trading Status 12.8K, O Operational Halt 12.6K, A Auction Information 2K, S System Event 6, X Official Price 6, D Security Directory 3. Useful as a sanity envelope: parser bugs that mis-attribute types or skip messages would show up as deviations from these ratios. The very low D count is surprising — most pre-market spin happens elsewhere; only intraday metadata changes hit the wire. The 6 X's = 3 symbols' open + close prints; modest because most listings aren't IEX-primary.
- **2026-05-11 — Heartbeat count is small in HIST.** Only 3,543 heartbeat packets in a 9.5 GB file (vs 274M total packets). Live feeds heartbeat continuously when no other traffic; in HIST the heartbeats appear concentrated in the pre-market quiet period before "Start of Messages". Once real traffic flows the data packets dominate.
