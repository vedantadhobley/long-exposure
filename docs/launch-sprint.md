...
- [x] **2-week backfill complete** (weeks 05-11 + 05-18 fully loaded). Catchup 2026-05-29 re-narrated all 12 days at v11 prompts + landed inter-day INTERPRET coverage. Replaces the 30-day backfill — see `decisions.md` 2026-05-25.
- [x] **Inter-day AGGREGATE** (weekly themes) shipping — `AggregateWeekWorkflow` runs per-week, content-addressed, reads the week's daily syntheses + 13 prior weekly rollups for trend context. `SynthesisVerifier` shared; dotted-ticker tokenization handled. `AggregateQuarterWorkflow` + `AggregateYearWorkflow` wired as cascade (gated, dormant until thresholds met). See `decisions.md` 2026-05-29.
- [x] Code audit complete — Round 3 structural pass closed the major items (Phase 7 halt_phase_span_label, Phase 7b BackgroundHeartbeat, Phase 8 PipelineWorkflow, Phase 9-A inter-day INTERPRET branch, Phase 9b DESCRIBE pattern-mislabel verifier check). `scripts/docs-check.sh` added as drift-prevention infra.
- [x] All docs current: AGENTS.md Active state, decisions.md, operations.md refreshed for Round 3; README + pipeline-architecture.md count-corrected by `scripts/docs-check.sh` first run.
...
| **2 — Weekly rollup machinery** | the real "rollup wiring" (AGGREGATE was own-week-only, manual, not in the pipeline). (a) prior-**8**-week window into `AggregateWeek` + verifier haystack/ticker-universe extended to the prior-week paragraphs; (b) **stateless rebuild** from this week's daily syntheses (never its own prior output); (c) **recompute-daily "week-so-far"** (upsert-by-`week_start`); (d) **content-address AGGREGATE** (`content_hash` skip); (e) wired into `DailyPipelineWorkflow` after SYNTHESIZE | ✅ **done 2026-05-26** (committed `b06da2b`; both 05-11/05-18 weeks verify clean, skip/recompute matrix confirmed, prompt v4 adds anti-cross-sum + no-priors guard) |
| **3 — Tiered numeric baselines** | daily cagg refresh window 30→400d (durable ~1yr exact baselines; invariant pinned both sides) + `BaselineProvider`/`CaggBaselineProvider` seam + `VolumeDeviationScorer` refactored onto it (behavior-preserving — 42-symbol set reproduced exactly) + explicit `RefreshBaselinesActivity` as the first Score-phase step (closes the "score-before-hourly-refresh" gap; no orchestration change). Monthly *numeric* cagg deferred — no months of data yet | ✅ **done 2026-05-26** (commits `afd9b25` + `e531d26`) |
| **4 — Polish batch + uniform cheap re-run** ◀ NEXT | breakdown/prompt fixes (iceberg `total_shares`, `BreakdownFmt` number formatting, dotted-ticker `SynthesisVerifier`/AGGREGATE fix, layering `orders_per_second`) → then re-run all days — **cheap now** (skip re-narrates only changed events). Also re-narrate 05-22 vol-dev (its breakdowns changed when the baseline window went 6→10 days) + re-score/narrate 05-18→21 for vol-dev uniformity. End-state: clean, uniform 2-week dataset | ~1 day (Fri) |
| **5 — Robustness (if time)** | ✅ `TimeInBookDriftScorer` (shipped 2026-05-27, durable lifetime baseline via materialize-step upsert); still open: `SchemaManager` concurrent-CREATE race; `isAlreadyIngested`-marks-ok-before-LLM fix; extract the full `MemoizedStage` abstraction (§6.7 phase 2) | partly done |
