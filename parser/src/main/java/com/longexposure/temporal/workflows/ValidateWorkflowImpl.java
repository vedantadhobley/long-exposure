...
public final class ValidateWorkflowImpl implements ValidateWorkflow {

    private static final Logger LOG = Workflow.getLogger(ValidateWorkflowImpl.class);
    ...
    @Override
    public RecordValidationActivity.Result run(final LocalDate date) {
        ...
        LOG.info("validate start  date={} (parallel)", date);
        ...
        LOG.info("validate done  date={} status={}", date, rec.status());
        return rec;
    }
    ...
    private static ValidationLegResult tryGet(final Promise<ValidationLegResult> p, final String legName) {
        try {
            ...
            LOG.error("validator leg failed  leg={} err={}", legName, af.getMessage());
            return null;
        }
    }
}
