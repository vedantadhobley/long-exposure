...
- **Phase**: Sprint 3 (LLM narration) in progress. Sprints 1+2 (Temporal scaffolding, scoring, selection) are done. Current pipeline shape: parse → validate → materialize order_lifecycle → score → select → narrate. Symbol enrichment + lifecycle-table materialization landed 2026-05-18. Next: cross-event linking + threshold-based selection + recursive Layer-0 expansion (see @docs/todo.md "Scoring + narration roadmap").
- **Implementation status** (as of 2026-05-18):
  ...
  - **Storage**: 14 hypertables + 4 standard tables. 13 wire-format hypertables (8 TOPS-side: trades / trade_breaks / quotes / status_events / auction_info / official_prices / securities / retail_liquidity; 5 DPLS-side: orders_add / orders_modify / orders_delete / orders_executed / clear_books). 1 derived hypertable: `order_lifecycle` (paired Add ↔ Delete + last Execute per order, materialized once per day between Parse and Score; partial index on `lifetime_ns < 100 ms` for sub-second PostCancel/Layering scans). 4 standard tables: `scored_events`, `selected_events`, `narratives`, `symbols`. 2026-05-08 fully loaded; DPLS↔TOPS trade aggregate: 9 134/9 134 symbols match, 0 share delta.
  - **Temporal pipeline**: 7 workflows on task queue `long-exposure-daily-pipeline` — `DailyPipelineWorkflow` (cron) + 6 ad-hoc (`ValidateOnly`, `MaterializeOnly`, `ScoreOnly`, `SelectOnly`, `NarrateOnly`, `RefreshSymbols`). 14 activities including `MaterializeOrderLifecycle` (added 2026-05-18 — pairs Add+Delete+Execute once per day so downstream scorers query the materialized table instead of running 13 GB hash JOINs per-scorer) and `RefreshSymbolMetadata` (weekly NASDAQ + IEX SecurityDirectory refresh of `symbols`). Cron schedules paused by default. Full layout in @docs/temporal-design.md.
  - **Scoring + selection**: 7 intraday `EventScorer`s in `com.longexposure.scoring.scorers.*` (halt, large_trade, sweep, post_cancel_cluster, layering, iceberg, liquidity_withdrawal). Push-model interface — bounded memory regardless of output count. PostCancel + Layering query `order_lifecycle` (sub-second sequential partial-index scan); the other 5 query raw wire-format tables directly. `Enrich.symbol()` joins per-ticker metadata (company name, exchange, ETF flag, round lot, prev close) into every breakdown from the in-memory `symbols` cache loaded at activity start. SelectTopEventsActivity pulls per-scorer top-N into `selected_events`. 2026-05-08 produces 660 K raw → 90 selected events.
  - **LLM narration (Sprint 3, in progress)**: `LlamaClient` (OkHttp + OpenAI-compatible chat, JVM-wide `Semaphore(2, fair)` enforcing 2-concurrent-call cap on `llama-large.joi`) + two-pass `BlueprintExtractor` → `ProseRenderer` → pure-code `GroundingVerifier` pipeline + `NarrateEventsActivity` + `NarrateWorkflow` for ad-hoc replay. Verifier uses `BigDecimal.stripTrailingZeros()` canonicalization so "$431.00" ↔ "431.0" ↔ "431" all match. Hits Qwen3.5-122B-A10B on joi. Narrations verified for all 7 scorer types. Full design in @docs/scoring-and-narration.md.
