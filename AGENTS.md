...
- **Implementation status** (as of 2026-05-22):
  ...
  - **Temporal pipeline**: 12 workflows on task queue `long-exposure-daily-pipeline`. 19 activities (`CompressChunksActivity` added 2026-05-23 to compress each day's chunks immediately after the LLM stages — 18× DB shrink via TimescaleDB columnstore, keeps disk at ~13 GB/day instead of ~230 GB/day uncompressed). Cron schedules paused by default. Full layout in @docs/temporal-design.md.
