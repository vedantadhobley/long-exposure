...
- **Phase**: Sprint 2 (event scoring + selection) complete. Pipeline shape: parse → validate → score → select → (narrate, next sprint). All 7 intraday scorers running on 2026-05-08 in ~45 min producing 660 K raw scored events, cut to 90 selected events for narration. Cron schedule registered paused for dev. Next sprint: LLM narration. See @docs/temporal-design.md for workflow layout and @docs/scoring-and-narration.md for the scoring + narration architecture.
